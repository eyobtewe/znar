...
class MusicApiService {
  ...
  Future<dynamic> _getter(String url) async {
    ...
    // String key = dotenv.env['xcaheader'];
  ...
  Future<List<Song>> fetchSongs(int page, int count) async {
    String url =
        '$ENDPOINT_SONGS?page=$page&limit=$count&$ENDPOINT_SONG_FILTER';
  ...
  Future<List<Album>> searchAlbums(String albumTitle) async {
    String url = '$ENDPOINT_SEARCH_ALBUMS$albumTitle&$ENDPOINT_ALBUM_FILTER';
  ...
  Future<List<Album>> fetchAlbums(int page, int count) async {
    String url =
        '$ENDPOINT_ALBUMS?page=$page&limit=$count&$ENDPOINT_ALBUM_FILTER';
  ...
  Future<List<Song>> fetchAlbumSongs(String albumId) async {
    String url =
        '$ENDPOINT_BASE/listings/albums/$albumId/songs/published?$ENDPOINT_SONG_FILTER';
  ...
  Future<List<Song>> fetchPlaylistSongs(String id) async {
    String url = '$ENDPOINT_PLAYLISTS_DETAIL/$id/songs?$ENDPOINT_SONG_FILTER';
  ...
  Future<Artist> fetchArtistDetails(String artistId) async {
    String url = '$ENDPOINT_ARTISTS_DETAIL/$artistId?$ENDPOINT_ALBUM_FILTER';
  ...
  Future<List<Song>> fetchArtistSongs(String artistId) async {
    String url =
        '$ENDPOINT_BASE/listings/artists/$artistId/songs/published?limit=50&$ENDPOINT_SONG_FILTER';
  ...
  Future<List<MusicVideo>> fetchArtistMusicVideos(
    ...
    if (musicVideoId != null && musicVideoId.length > 5) {
      url =
          '$ENDPOINT_BASE/listings/artists/$artistId/videos/published?_id[ne]=$musicVideoId&$ENDPOINT_MUSIC_VIDEO_FILTER';
    } else {
      url =
          '$ENDPOINT_BASE/listings/artists/$artistId/videos/published?$ENDPOINT_MUSIC_VIDEO_FILTER';
    }
    final parsedJson = await _getter(url);
    List<MusicVideo> data = [];
    parsedJson.forEach((var item) {
  ...
  Future<List<Artist>> searchArtist(String artistName) async {
    String url = '$ENDPOINT_SEARCH_ARTISTS$artistName&$ENDPOINT_ARTIST_FILTER';
  ...
  Future<List<Album>> fetchArtistAlbums(String artistId) async {
    String url =
        '$ENDPOINT_BASE/listings/artists/$artistId/albums/published?$ENDPOINT_ALBUM_FILTER';
  ...
  Future<List<Artist>> fetchArtists(int page, int count) async {
    String url =
        '$ENDPOINT_ARTISTS?page=$page&limit=$count&$ENDPOINT_ARTIST_FILTER';
  ...
  Future<MusicVideo> fetchMusicVideoDetails(String id) async {
    String url = '$ENDPOINT_MUSIC_VIDEOS/$id?$ENDPOINT_MUSIC_VIDEO_FILTER';
    final parsedJson = await _getter(url);
  ...
  Future<List<MusicVideo>> fetchMusicVideos(int page, int count) async {
    String url =
        '$ENDPOINT_MUSIC_VIDEOS?page=$page&limit=$count&$ENDPOINT_MUSIC_VIDEO_FILTER';
